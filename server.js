...
const MongoStore = MongoStorePkg.default || MongoStorePkg.MongoStore || MongoStorePkg;
...
// FIX 1: collab schema must include ALL team fields.
// Original only had tasks/chat/activity/announcements —
// Mongoose silently stripped teams/teamRoles/etc on every save.
const stateSchema = new mongoose.Schema(
  {
    ...
    collab: {
      ...
      teamMemberships: { type: Array, default: [] },
      teamTasks:       { type: Array, default: [] },
      teamChats:       { type: Array, default: [] },
    }
  },
  { versionKey: false }
);
...
function createEmptyCollab() {
  return { tasks: [], chat: [], activity: [], announcements: [], teams: [], teamRoles: [], teamMemberships: [], teamTasks: [], teamChats: [] };
}
...
function migrateLegacyCollab(collab, users) {
  ...
  base.teams = [{ id: teamId, name: "General Team", department: "", managerId: firstManager ? firstManager.id : null, created: new Date().toISOString() }];
  base.teamRoles = [
  ...
  base.teamMemberships = firstManager ? [{ id: `${teamId}:${firstManager.id}`, teamId, userId: firstManager.id, teamRoleId: "tr-manager", created: new Date().toISOString() }] : [];
  base.teamTasks = Array.isArray(base.tasks) ? base.tasks.map((t) => ({ ...t, teamId: t.teamId || teamId })) : [];
  base.teamChats = Array.isArray(base.chat)  ? base.chat.map((m)  => ({ ...m, teamId: m.teamId || teamId, channelType: m.channelType || "group" })) : [];
  return base;
}

function docToUser(doc) {
  return { id: doc.id, username: doc.username, email: doc.email, role: normalizeRole(doc.role), status: doc.status, created: doc.created, tasks: doc.tasks || [], notifications: doc.notifications || [] };
}

async function ensureSeedData() {
  const admin = await User.findOne({ id: "nexus-admin-root" }).lean();
  if (!admin) {
    await User.create({ id: "nexus-admin-root", username: "admin", email: "admin@nexus.io", password_hash: await bcrypt.hash("admin2077", 10), role: "admin", status: "active", created: new Date().toISOString(), tasks: [], notifications: [] });
  }
...
app.use((req, res, next) => {
  ...
  if (origin) { res.header("Access-Control-Allow-Origin", origin); res.header("Vary", "Origin"); res.header("Access-Control-Allow-Credentials", "true"); }
...
async function requireAuth(req, res, next) {
  ...
  if (!user) { req.session.destroy(() => {}); return res.status(401).json({ error: "Invalid session" }); }
...
function normalizeCollabForPersist(collab, authUser, currentCollab) {
  ...
  // Strip base64 dataUrls from attachments to avoid hitting MongoDB 16MB limit
  if (Array.isArray(next.teamTasks)) {
    next.teamTasks = next.teamTasks.map((t) => ({
      ...t,
      attachments: Array.isArray(t.attachments)
        ? t.attachments.map((a) => typeof a === "object" && a !== null ? { id: a.id, name: a.name, mime: a.mime } : a)
        : []
    }));
  }
...
async function saveState(input, authUser) {
  ...
  for (const u of incomingUsers) {
    ...
    const payload = { id: u.id, username: u.username, email: u.email, role: normalizeRole(u.role || "employee"), status: u.status || "pending", created: u.created || new Date().toISOString(), tasks: u.tasks || [], notifications: u.notifications || [] };
    if (existingById.has(u.id)) {
      await User.updateOne({ id: u.id }, { $set: payload });
    } else {
      await User.create({ ...payload, password_hash: await bcrypt.hash("changeme123", 10) });
    }
  }

  for (const u of existing) {
    if (!canEditUsers) continue;
    if (u.id !== "nexus-admin-root" && !incomingIds.has(u.id)) await User.deleteOne({ id: u.id });
  }

  const stateDoc        = await AppState.findOne({ id: "global" }).lean();
  const persistedCollab = normalizeCollabForPersist(input.collab, authUser, stateDoc?.collab || createEmptyCollab());

  // FIX 2: dot-notation $set per field — prevents Mongoose from re-casting
  // the whole collab object through the schema (which silently drops unknown fields).
  await AppState.updateOne(
    { id: "global" },
    { $set: {
      "collab.tasks":           persistedCollab.tasks           || [],
      "collab.chat":            persistedCollab.chat            || [],
      "collab.activity":        persistedCollab.activity        || [],
      "collab.announcements":   persistedCollab.announcements   || [],
      "collab.teams":           persistedCollab.teams           || [],
      "collab.teamRoles":       persistedCollab.teamRoles       || [],
      "collab.teamMemberships": persistedCollab.teamMemberships || [],
      "collab.teamTasks":       persistedCollab.teamTasks       || [],
      "collab.teamChats":       persistedCollab.teamChats       || [],
    }},
    { upsert: true }
  );
}
...
app.post("/api/auth/signup", async (req, res) => {
  ...
  const id = "u-" + Date.now();
  await User.create({ id, username: u, email: e, password_hash: await bcrypt.hash(password, 10), role: "employee", status: "pending", created: new Date().toISOString(), tasks: [], notifications: [] });
...
app.post("/api/auth/change-password", requireAuth, async (req, res) => {
  ...
  if (!user || !(await bcrypt.compare(currentPassword, user.password_hash))) return res.status(400).json({ error: "Wrong current password" });
...
app.get("/api/state", requireAuth, async (req, res) => { res.json(await loadState()); });

// FIX 3: users field is now optional in the payload.
// Original required users to always be an array, causing errors for non-admin saves.
app.put("/api/state", requireAuth, async (req, res) => {
  const state = req.body || {};
  if (typeof state.collab !== "object" || state.collab === null) return res.status(400).json({ error: "Invalid state payload" });
  if (state.users !== undefined && !Array.isArray(state.users)) return res.status(400).json({ error: "Invalid users payload" });
